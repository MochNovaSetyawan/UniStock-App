...
$cStmt = $pdo->prepare("SELECT COUNT(*) FROM audit_logs al WHERE $whereStr");
...
<div class="card">
  <div class="table-toolbar">
    <form method="GET" style="display:contents;">
      <div class="search-box">
        ...
        <input type="text" name="search" value="<?= Format::escape($search) ?>" placeholder="Cari aksi, deskripsi...">
      </div>
      ...
      <select name="user_id" class="filter-select" onchange="this.form.submit()">
        ...
        <option value="<?= $u['id'] ?>" <?= $userId == $u['id'] ? 'selected' : '' ?>><?= Format::escape($u['full_name']) ?></option>
        <?php endforeach; ?>
      </select>
  ...
  <div class="table-container">
    ...
    <table class="table">
      ...
      <tbody>
        ...
        ?>
        <tr>
          <td class="td-meta"><?= Format::datetime($log['created_at']) ?></td>
          <td>
            <div class="table-item-name"><?= Format::escape($log['full_name'] ?? 'System') ?></div>
            <div class="table-item-code">@<?= Format::escape($log['username'] ?? '-') ?></div>
          </td>
          <td><span class="badge badge-<?= $ac ?>"><?= Format::escape($log['action']) ?></span></td>
          <td><span class="badge badge-secondary"><?= Format::escape($log['module']) ?></span></td>
          <td style="max-width:300px;"><?= Format::escape($log['description'] ?? '-') ?></td>
          <td class="td-meta"><?= Format::escape($log['ip_address'] ?? '-') ?></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
  ...
  <?= Paginator::render($total, $perPage, $page, '?' . http_build_query(array_filter(['search' => $search, 'module' => $module, 'user_id' => $userId]))) ?>
</div>
