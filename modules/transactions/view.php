...
$stmt = $pdo->prepare("
    SELECT t.*, i.name as item_name, i.code as item_code, i.image as item_image,
           ...
           u4.full_name as return_requested_by_name,
    ...
    LEFT JOIN users u4 ON t.return_requested_by = u4.id
...
$stmt->execute([$id]);
$tx = $stmt->fetch();
if (!$tx) { Session::flash('error', 'Transaksi tidak ditemukan.'); header('Location: index.php'); exit; }

// Workers can only see their own
if (Auth::hasRole('worker') && $tx['requested_by'] != Auth::id()) {
    Session::flash('error', 'Akses ditolak.'); header('Location: index.php'); exit;
}

// Load linked units
$txUnitStmt = $pdo->prepare("
...
<div class="page-header">
  <div class="page-title">
    ...
    <p class="mono"><?= Format::escape($tx['code']) ?></p>
  </div>
  <div class="btn-group">
    ...
    <?php endif; ?>
    <?php if (Auth::isAdmin() && $tx['status'] === 'active'): ?>
    ...
    <a href="request_return.php?id=<?= $tx['id'] ?>" class="btn btn-primary btn-sm">Ajukan Pengembalian</a>
    <?php endif; ?>
    ...
    <form method="POST" action="approve.php" style="display:inline;" onsubmit="return confirm('Setujui pengajuan pengembalian ini?')">
      ...
      <button type="submit" class="btn btn-success btn-sm">Setujui Pengembalian</button>
    </form>
    <form method="POST" action="approve.php" style="display:inline;" onsubmit="return confirm('Tolak pengajuan pengembalian?')">
...
<div class="grid-2" style="gap:20px;align-items:start;">
  <div>
      ...
      <div class="card-body">
        <div style="display:flex;gap:14px;align-items:center;">
          <div style="width:56px;height:56px;background:var(--bg-elevated);border-radius:var(--radius-sm);overflow:hidden;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            ...
            <img src="<?= UPLOAD_URL . Format::escape($tx['item_image']) ?>" style="width:100%;height:100%;object-fit:cover;">
            <?php else: ?>
          ...
          <div>
            <a href="<?= APP_URL ?>/modules/inventory/view.php?id=<?= $tx['item_id'] ?>" style="font-size:1rem;font-weight:600;color:var(--text-primary);"><?= Format::escape($tx['item_name']) ?></a>
            <div class="mono" style="font-size:0.8rem;color:var(--text-muted);"><?= Format::escape($tx['item_code']) ?></div>
          </div>
        </div>
        ...
        <div class="detail-grid">
          ...
          <?php if ($tx['from_location']): ?><div class="detail-field"><label>Dari</label><span><?= Format::escape($tx['from_location']) ?></span></div><?php endif; ?>
          <?php if ($tx['to_location']): ?><div class="detail-field"><label>Ke</label><span><?= Format::escape($tx['to_location']) ?></span></div><?php endif; ?>
        </div>
      </div>
    </div>
    ...
    <div class="card mb-20">
      ...
      <div class="card-body">
        <div class="detail-grid">
          <div class="detail-field"><label>Nama</label><span><?= Format::escape($tx['borrower_name'] ?: '-') ?></span></div>
          <div class="detail-field"><label>ID / NIM / NIP</label><span><?= Format::escape($tx['borrower_id_number'] ?: '-') ?></span></div>
          <div class="detail-field"><label>Departemen</label><span><?= Format::escape($tx['borrower_department'] ?: '-') ?></span></div>
          <div class="detail-field"><label>No. Telepon</label><span><?= Format::escape($tx['borrower_phone'] ?: '-') ?></span></div>
        </div>
        <?php if ($tx['purpose']): ?>
        <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border);">
          <label class="form-label" style="display:block;margin-bottom:6px;">Keperluan</label>
          <p style="font-size:0.85rem;color:var(--text-secondary);"><?= nl2br(Format::escape($tx['purpose'])) ?></p>
        </div>
        <?php endif; ?>
      </div>
    </div>
        ...
        <table class="table" style="margin:0;">
          <thead>
            <tr>
              ...
              <th>Lokasi</th>
              <th>Status</th>
              <?php if (in_array($tx['status'], ['returned', 'return_requested'])): ?><th>Kondisi Laporan</th><?php endif; ?>
            </tr>
          </thead>
          <tbody>
            <?php foreach ($txUnits as $u): ?>
            <tr>
              <td><span class="mono" style="color:var(--accent);font-weight:600;"><?= Format::escape($u['full_code']) ?></span></td>
              <td style="color:var(--text-muted);"><?= $u['serial_number'] ? Format::escape($u['serial_number']) : '-' ?></td>
              <td><?= $u['loc_name'] ? Format::escape($u['loc_name']) : '-' ?></td>
              <td><?= Badge::unitStatus($u['unit_status']) ?></td>
              <?php if (in_array($tx['status'], ['returned', 'return_requested'])): ?>
              <td>
                <?php if ($u['return_condition']): ?>
                  <?= Badge::condition($u['return_condition']) ?>
                  <?php if ($u['return_notes']): ?>
                  <div class="table-item-code" style="margin-top:3px;"><?= Format::escape($u['return_notes']) ?></div>
                  <?php endif; ?>
                <?php else: ?>-<?php endif; ?>
              </td>
              <?php endif; ?>
    ...
    <?php if ($tx['status'] === 'return_requested'): ?>
    <div class="card mb-20" style="border-color:rgba(59,130,246,0.3);">
      <div class="card-header"><div class="card-title" style="color:var(--info);">Pengajuan Pengembalian</div></div>
      <div class="card-body">
        <div class="detail-grid">
          <div class="detail-field"><label>Diajukan oleh</label><span><?= Format::escape($tx['return_requested_by_name'] ?? '-') ?></span></div>
          <div class="detail-field"><label>Waktu Pengajuan</label><span><?= Format::datetime($tx['return_requested_at'] ?? '') ?></span></div>
          <div class="detail-field"><label>Kondisi Dilaporkan</label><span><?= $tx['return_condition'] ? Badge::condition($tx['return_condition']) : '-' ?></span></div>
        </div>
        <?php if ($tx['return_notes']): ?>
        <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border);">
          <label class="form-label" style="display:block;margin-bottom:6px;">Catatan dari Pekerja</label>
          <p style="font-size:0.85rem;color:var(--text-secondary);"><?= nl2br(Format::escape($tx['return_notes'])) ?></p>
        </div>
        <?php endif; ?>
      </div>
    </div>
    ...
    <div class="card" style="border-color:rgba(16,185,129,0.3);">
      ...
      <div class="card-body">
        <div class="detail-grid">
          <div class="detail-field"><label>Dikembalikan</label><span><?= Format::datetime($tx['actual_return']) ?></span></div>
          <div class="detail-field"><label>Kondisi</label><span><?= Badge::condition($tx['return_condition'] ?? 'good') ?></span></div>
          <div class="detail-field"><label>Diterima oleh</label><span><?= Format::escape($tx['returned_by_name'] ?? '-') ?></span></div>
        </div>
        <?php if ($tx['return_notes']): ?>
        <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border);">
  ...
  <div>
      ...
      <div class="activity-list" style="padding:0 20px;">
        <div class="activity-item">
          ...
          <div class="activity-content">
            ...
            <div class="activity-meta"><?= Format::escape($tx['requested_by_name'] ?? '-') ?> &bull; <?= Format::datetime($tx['created_at']) ?></div>
          </div>
        </div>
        ...
        <div class="activity-item">
          ...
          <div class="activity-content">
            ...
            <div class="activity-meta"><?= Format::escape($tx['approved_by_name'] ?? '-') ?> &bull; <?= Format::datetime($tx['approved_at']) ?></div>
          </div>
        </div>
        <?php endif; ?>
        ...
        <div class="activity-item">
          ...
          <div class="activity-content">
            ...
            <div class="activity-meta"><?= Format::datetime($tx['expected_return']) ?></div>
          </div>
        </div>
        <?php endif; ?>
        <?php if (!empty($tx['return_requested_at'])): ?>
        <div class="activity-item">
          <div class="activity-dot blue"></div>
          <div class="activity-content">
            <div class="activity-title">Pengajuan pengembalian dikirim</div>
            <div class="activity-meta"><?= Format::escape($tx['return_requested_by_name'] ?? '-') ?> &bull; <?= Format::datetime($tx['return_requested_at']) ?></div>
          </div>
        </div>
        <?php endif; ?>
        ...
        <div class="activity-item">
          ...
          <div class="activity-content">
            <div class="activity-title">Pengembalian disetujui &amp; selesai</div>
            <div class="activity-meta"><?= Format::escape($tx['returned_by_name'] ?? '-') ?> &bull; <?= Format::datetime($tx['actual_return']) ?></div>
          </div>
        </div>
        <?php endif; ?>
      </div>
    ...
    <div class="card mt-20">
      ...
      <div class="card-body" style="font-size:0.85rem;color:var(--text-secondary);"><?= nl2br(Format::escape($tx['notes'])) ?></div>
    </div>
    <?php endif; ?>
  </div>
</div>
