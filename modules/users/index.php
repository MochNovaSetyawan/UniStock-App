...
$cStmt = $pdo->prepare("SELECT COUNT(*) FROM users WHERE $whereStr");
$cStmt->execute($params);
$total = (int)$cStmt->fetchColumn();

$stmt = $pdo->prepare("SELECT * FROM users WHERE $whereStr ORDER BY created_at DESC LIMIT ? OFFSET ?");
...
<div class="card">
  <div class="table-toolbar">
    <form method="GET" style="display: contents;">
      <div class="search-box">
        ...
        <input type="text" name="search" value="<?= Format::escape($search) ?>" placeholder="Cari nama, username, email...">
      </div>
        ...
        <tr>
          <td>
            <div style="display: flex; align-items: center; gap: 12px;">
              ...
              <div>
                <div class="table-item-name"><?= Format::escape($u['full_name']) ?></div>
                <div class="table-item-code">@<?= Format::escape($u['username']) ?> &bull; <?= Format::escape($u['email']) ?></div>
              </div>
            </div>
          </td>
          <td><?= Badge::role($u['role']) ?></td>
          <td><?= Format::escape($u['department'] ?: '-') ?></td>
          <td>
            <span class="badge <?= $u['is_active'] ? 'badge-success' : 'badge-secondary' ?>">
              <?= $u['is_active'] ? 'Aktif' : 'Nonaktif' ?>
            </span>
          </td>
          <td class="td-meta"><?= $u['last_login'] ? Format::datetime($u['last_login']) : 'Belum pernah' ?></td>
            ...
            <div class="btn-group" style="justify-content:flex-end;">
              ...
              <form method="POST" action="delete.php" style="display:inline;">
                ...
                <button type="button" class="btn btn-ghost btn-icon btn-sm" style="color:var(--danger);" title="Hapus"
                  onclick="confirmDelete('Hapus user <?= Format::escape(addslashes($u['full_name'])) ?>?', this.closest('form'))">
  ...
  <?= Paginator::render($total, $perPage, $page, '?' . http_build_query(array_filter(['search' => $search, 'role' => $role]))) ?>
</div>
